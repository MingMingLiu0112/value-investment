...
项目只维护根目录的 `A股价值投资_Agent前端智能跟踪模板.xlsx`。同步会原位更新观察名单、财务指标、估值跟踪、仓位管理和数据源审计页；不会生成第二份日常工作簿。每次同步前会在 `runtime/workbook-backups/` 留一份仅用于故障回退的技术备份，审计页按来源文件、股票、字段和报告期去重。

本地电脑通过 Tailscale 私网以只读账户连接中央 PostgreSQL；服务器只允许 Tailscale 访问数据库。开始自动同步前，先在 Tailscale 客户端登录同一 tailnet，并关闭正在编辑的 Excel 文件。
