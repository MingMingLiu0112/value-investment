#!/usr/bin/env python3
"""Recompute acceptance evidence for the protected M3 original-workbook candidate."""
...
from value_investment_agent.m3_original_workbook_acceptance_audit import (  # noqa: E402
...
DEFAULT_SOURCE = ROOT / "A股价值投资_Agent前端智能跟踪模板.xlsx"
DEFAULT_CANDIDATE = (
...
DEFAULT_WPS_RECEIPT = (
    ROOT / "runtime" / "m3-original-decision-review-20260924" / "wps-verification.json"
)
DEFAULT_CANONICAL_WPS = (
    Path(r"C:\Users\we\WPSDrive\197617831\WPS云盘\价投跟踪")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--canonical-wps", type=Path, default=DEFAULT_CANONICAL_WPS)
...
def main(argv: list[str] | None = None) -> int:
    ...
    spec = M3OriginalWorkbookAcceptanceSpec(
        ...
        canonical_wps_path=args.canonical_wps,
        canonical_sha256=EXPECTED_SOURCE_SHA256,
    )
    test_evidence = _run_offline_tests(root) if args.run_tests else None
    receipt = audit(
        root,
        spec,
    ...
    if args.json_only:
        ...
        return 0
    print(f"M3 original-workbook acceptance status: {receipt['status']}")
