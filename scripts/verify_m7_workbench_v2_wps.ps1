...
if ($package.schema_version -ne "m7-workbench-candidate-v2") {
    throw "Unexpected M7 v2 manifest schema: $($package.schema_version)"
}
...
try {
    ...
    if ($book.Worksheets.Count -ne 96) {
    ...
    $receipt = @{
        status = "passed"
        mode = "protected_m7_workbench_v2_display_candidate"
        workbook = $path
        sha256 = $before
        ...
        action = "no_order"
        check_scope = "M7 v2 WPS read-only open/read/calculate, formula-error scan, presentation sheet order, M4/M5 joint navigation, no_order text and canonical hash; no visual click verification"
    }
