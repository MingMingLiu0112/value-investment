...
def test_detailed_statements_supply_cash_debt_and_free_cash_flow(monkeypatch) -> None:
    balance = Frame([
        Row({
            ...
            "资产总计": 10_000_000_000, "负债合计": 4_000_000_000,
        })
    ])
    cashflow = Frame([
        Row({
    ...
    income = Frame([Row({"报告日": "20260630", "营业成本": 3_000_000_000})])

    def report(stock: str, symbol: str):
        return {"资产负债表": balance, "现金流量表": cashflow, "利润表": income}[symbol]
    ...
    assert values["cash"] == 5_000_000_000
    cash_records = [record for record in records if record.field_name == 'cash']
    assert len(cash_records) == 1
    assert cash_records[0].unit == 'CNY'
    assert cash_records[0].point_metadata['source_line_item'] == '货币资金'
    assert values["interest_bearing_debt"] == 10
    assert values["free_cash_flow"] == 14
    assert values["total_assets"] == 10_000_000_000
    assert values["total_liabilities"] == 4_000_000_000
    assert values["operating_cost"] == 3_000_000_000
