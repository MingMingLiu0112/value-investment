...
def test_offline_historical_runner_never_requires_a_live_quote_or_next_trade_day():
    ...
    assert "build_moutai_historical_validation_admission.py" in script
    assert "audit_historical_validation_receipt.py" in script
    ...
    assert "validation_classification = $admission.classification" in script
    assert "receipt_audit_status = $receiptAudit.status" in script
