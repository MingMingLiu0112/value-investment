...
## 2026-09-24 原 Excel 集成更新

同一 AC8 报告 JSON 已并入 M2 原工作簿候选并受保护发布。发布前原表 SHA-256 为
`a62a6ae634ea949db36c3c209278515e2ee66ef3a61aaa25d59d2051d5954d58`；候选和发布后
WPS 原表 SHA-256 均为
`64c8deff1a237076d2ba0b00afc8905d23bd9d117cb132dfc6757071b5659911`。

- 新增 `11_研究报告`、`12_研究证据` 两页，共 55 个工作表。
- 研究报告页 22 行，证据页 245 行，识别 228 个来源链接。
- WPS 发布前/发布后只读收据均为 `passed`；真实引擎路径为
  `D:\WPS Office\12.1.0.28505\office6`。
- 回退原件与发布收据见
  `runtime/workbook-backups/stage-frontend-5e6ab310df554a49a100ccbcc6d68c33/`。
- AC8 仍为 `AC8_REVIEW_PENDING`；AC10 机器检查通过但用户视觉审核待完成，M2 仍为
  `PARTIAL`。

以下内容记录上一阶段研究报告生成；其中“本轮未新增 Excel 快照”只描述当时状态，不适用于
本节之后的原表集成发布。
