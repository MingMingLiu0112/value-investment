...
## 2026-09-24 M5 贵州茅台真实披露队列工作簿

新增一个受 Git 跟踪的公开候选，只发布 600519 的真实 CNINFO 待复核队列，
不覆盖 canonical 或既有 M5 三家公司队列。
...
- 工作表：`00_总览`、`01_待复核公告`、`02_来源覆盖`、`03_输入与边界`。
- WPS 只读收据：
...
在不覆盖既有 v1 工作簿的前提下，新增一个由 600519 真实 CNINFO 队列生成的空白人工
复核输入包。它只增加关系字段和归档原件 hyperlink，所有人工材料性判定仍为空。

- 文件：
  `A股价值投资_M5真实披露人工复核回填_600519_20260924_v2.xlsx`
- 字节数：12,642
- SHA-256：
  `dc0880d1bc492c8256b7e543b837cc9ae750a428b982691c1efa2f36cd88c7ef`
- manifest SHA-256：
  `aacfbe979ff6f60ac245d1332ffc32991207520838d7fbc12f2fc435fa54f2a6`
- 9 条候选的归档 PDF 均现场重算并与队列记录一致；WPS 云盘同名副本逐字节一致。
- 新增 `替代事件ID`、`事件簇ID` 两列；旧 13 列工作簿仍可读取。
- `action=no_order`，不生成事件、通知、仓位、订单或生产授权。
