...
2. manifest 已具备代码版本、配置清单和发布 Excel Hash；仍需在真实生产
   备份作业中启用并验证。
3. 尚未执行一次真实生产快照到新库的恢复演练，因此没有可证明的 RPO/RTO。部署脚本会在 `/etc/value-investment-agent/restore-postgres.env` 创建权限为 600 的独立恢复库密码，并更新 `agent.env` 的隔离 `RESTORE_DATABASE_URL`；恢复容器不复用生产库密码。恢复收据的 RTO 从容器启动前计时，另记数据库恢复操作耗时。首次或既有安装都须先核对这两个配置及隔离库身份；未经具体生产授权不运行服务器演练或启用 timer。
